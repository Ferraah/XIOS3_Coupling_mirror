...
contains
    ...
    subroutine load_toymodel_data(config)
        ...
        if(model_id=="atm") then 
            ! @todo SET double freq
            CALL xios_get_field_attr("field2D_atm_to_oce", freq_op=tmp2)
        else if (model_id == "ocn") then
            CALL xios_get_field_attr("field2D_atm_to_oce", freq_op=tmp2)
        else if (model_id == "ice") then
            CALL xios_get_field_attr("field2D_atm_to_ice", freq_op=tmp2)
        end if
        ...
        print *, trim(model_id)//" frequency of operation: ", config%freq_op_in_ts
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    implicit none
        ...
        if(model_id=="atm") call xios_set_domain_attr("domain", ni_glo=config%ni_glo, nj_glo=config%nj_glo, type=config%field_type)
    ...
    implicit none 
        ...
        if (model_id == "atm") allocate(field_send(conf%ni_glo, conf%nj_glo))
        ...
        do while (conf%curr_date < conf%end_date)
            ...
            if (model_id=="atm") then
                ...
                print *, "ATM: sending field @ts=", curr_timestep, " with value ", field_send(1,1)
                ...
                if(curr_timestep==1) then
                    ...
                    if (model_id=="ocn") then 
                        ...
                        print *, "  ICE: receiving restart field @ts=", curr_timestep, " with value ", field_recv(1,1)
                    end if
                    ...
                    if (model_id == "ocn") then
                        print *, "  OCN: receiving field @ts=", curr_timestep, " with value ", field_recv(1,1)
                    else if (model_id == "ice") then
                        print *, "  ICE: receiving field @ts=", curr_timestep, " with value ", field_recv(1,1)
                    end if
