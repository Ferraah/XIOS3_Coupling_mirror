...
contains
    ...
    subroutine init_domain(local_comm, domain_id, cpl_conf, cpl_conf_domain, fd)
        ...
        if (trim(cpl_conf_domain) == "bggd") then
            call init_domain_lmdz(local_comm, domain_id, cpl_conf, cpl_conf_domain, fd)
        else if (trim(cpl_conf_domain) == "icoh") then
            call init_domain_icos(local_comm, domain_id, cpl_conf, cpl_conf_domain, fd)
        else if (trim(cpl_conf_domain) == "nogt" .or. trim(cpl_conf_domain) == "torc" .or. trim(cpl_conf_domain) == "t12e") then
            call init_domain_orca(local_comm, domain_id, cpl_conf, cpl_conf_domain, fd)
        else if(trim(cpl_conf_domain) == "lmdz") then
            call init_domain_lmdz(local_comm, domain_id, cpl_conf, cpl_conf_domain, fd)
        else 
            print *, "Unknown domain type: ", trim(cpl_conf_domain), " at line ", __LINE__
            stop 1
        end if
    ...
    subroutine init_domain_orca(local_comm, domain_id, cpl_conf, cpl_conf_domain, fd)
        ...
        logical, allocatable :: mask(:)
        integer :: nbp, nbp_glo, offset_i, offset_j
        ...
        DO j=0,nj-1
            DO i=0,ni-1
                ij=j*ni+i
                lon2d(i, j)=lon_glo2d(ibegin+i,jbegin+j)
                lat2d(i,j)=lat_glo2d(ibegin+i,jbegin+j)
        ...
        ! print how many 1 and 0 in mask(ij)
        print *, "Number of .true. in mask: ", count(mask)
        print *, "Number of .false. in mask: ", size(mask) - count(mask)
        ...
        if (xios_is_valid_domain(trim(domain_id))) then
            ...
            ! call xios_set_domain_attr(trim(domain_id), data_ibegin=-offset_i, data_ni=ni+2*offset_i)
            ! call xios_set_domain_attr(trim(domain_id), data_jbegin=-offset_j, data_nj=nj+2*offset_j)
         ...
         CALL xios_set_domain_attr(trim(domain_id), radius=1.0_8)
            ...
            print *, "ORCA type set in XIOS: ", trim(domain_id)
            print *, "ni_glo: ", ni_glo
            print *, "nj_glo: ", nj_glo
            print *, "ni: ", ni
            print *, "nj: ", nj
    ...
    subroutine init_domain_bggd(local_comm, domain_id, cpl_conf, file_domain_id, fd)
        ...
        IF (xios_is_valid_domain(TRIM(domain_id))) THEN
            ...
            CALL xios_set_domain_attr(TRIM(domain_id), lonvalue_2d=lon2d, latvalue_2d=lat2d, mask_1d=mask)
            CALL xios_set_domain_attr(TRIM(domain_id), bounds_lon_2d=bounds_lon, bounds_lat_2d=bounds_lat, nvertex=ncrn)
#ifdef USE_SURFACE
            CALL xios_set_domain_attr(TRIM(domain_id), area=srf_loc2d)
            CALL xios_set_domain_attr(TRIM(domain_id), radius=1.0_8)
            ...
            print *, "BGGD type setted in XIOS: ", trim(domain_id)
            print *, "ni_glo: ", ni_glo
            print *, "nj_glo: ", nj_glo
            print *, "ni: ", ni
            print *, "nj: ", nj
...
!             call xios_set_domain_attr(trim(domain_id), ni_glo=ni_glo)
...
! #endif
        ...
        IF (xios_is_valid_domain(TRIM(domain_id))) THEN
            ...
            print *, "ICOS type set in XIOS: ", trim(domain_id)
            print *, "ni_glo: ", ni_glo
            print *, "nj_glo: ", 1
            print *, "ni: ", nbp
            print *, "nj: ", 1
...
!> @brief Initialize a LMDZ domain, distributing points among processes and setting XIOS domain attributes.
!! @param[in] local_comm MPI communicator of the model
!! @param[in] domain_id Domain id of the tag in iodef.xml
!! @param[in] cpl_conf Coupling configuration retrieved previously
!! @param[in] cpl_conf_domain Identifier for the lmdz grid to be loaded from grid file
!! @param[out] fd Description of the local distribution of the field
subroutine init_domain_lmdz(local_comm, domain_id, cpl_conf, cpl_conf_domain, fd)
    implicit none
    ...
    ! Global variables to retrieve data from file in 2D
    double precision, pointer :: lon_glo2d(:,:), lat_glo2d(:,:), lon_glo(:), lat_glo(:)
    double precision, pointer :: clo_glo2d(:,:,:), cla_glo2d(:,:,:)
    integer, pointer :: msk_glo2d(:,:)
    double precision, pointer :: srf_glo2d(:,:)
    ...
    double precision, allocatable :: lon2d(:,:), lat2d(:,:)
    logical, allocatable :: mask(:)
    logical, allocatable :: dom_mask(:)
    integer :: i, j, ij
    ...
    allocate(lon(0:ni-1), lat(0:nj-1))
    allocate(mask(0:ni*nj-1))
    allocate(lon_glo(0:ni_glo-1), lat_glo(0:nj_glo-1))

    do i = 0, ni-1
        lon(i) = lon_glo2d(ibegin + i, jbegin)
    end do
    do j = 0, nj-1
        lat(j) = lat_glo2d(ibegin, jbegin + j)
    end do
    ...
    if (xios_is_valid_domain(trim(domain_id))) then
        call xios_set_domain_attr(trim(domain_id), type="rectilinear", ni_glo=ni_glo, ibegin=ibegin, ni=ni, nj_glo=nj_glo, &
            jbegin=jbegin, nj=nj)
        call xios_set_domain_attr(trim(domain_id), data_dim=2, lonvalue_1d=lon, latvalue_1d=lat, mask_1d=mask)
        call xios_set_domain_attr(trim(domain_id), radius=1.0_8)
    end if

    ! Deallocate global pointers
    deallocate(lon_glo2d, lat_glo2d, clo_glo2d, cla_glo2d, msk_glo2d, srf_glo2d)
    deallocate(lon, lat, mask)
