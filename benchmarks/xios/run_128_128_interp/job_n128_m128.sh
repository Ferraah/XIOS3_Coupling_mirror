...
#SBATCH --output=../xios/results_interp/out_n128_m128.txt
#SBATCH --error=../xios/results_interp/err_n128_m128.txt
#SBATCH --ntasks=256
...
rm -rf ../outputs_interp/interpolations_times_n128_m128.txt

for i in $(seq 1 20); do
    echo "Running interpolation iteration $i"
    mpirun -np 128 ../12_ping_pong.exe oce true : -np 128 ../12_ping_pong.exe atm true > ../outputs_interp/ocean_times_n128_m128.txt

    # Add the time taken for interpolation from xios log file
    grep "compute" xios_client_*.out | awk -F " " '{print $8}'  >> ../outputs_interp/interpolations_times_n128_m128.txt
done
