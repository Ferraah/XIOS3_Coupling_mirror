...
#SBATCH --output=results/out_n256_m256.txt
#SBATCH --error=results/err_n256_m256.txt
#SBATCH --ntasks=512
...
echo "Running the ping-pong test with N=256 and M=256"
mpirun -np 256 ../12_ping_pong.exe oce t12e LR : -np 256 ../12_ping_pong.exe atm icoh U > ../outputs/ocean_times_n256_m256.txt


# If interpolation is enabled, extract timing
if [[ "$1" == "true" ]]; then
    # Add the time taken for interpolation to output
    grep "compute" xios_client_*.out | awk -F " " '{print $8}'  > ../outputs/interpolations_times_n256_m256.txt
fi