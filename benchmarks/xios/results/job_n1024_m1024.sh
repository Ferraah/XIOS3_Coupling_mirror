#!/bin/bash
#SBATCH --job-name=bench_n1024_m1024
#SBATCH --output=results/out_n1024_m1024.txt
#SBATCH --error=results/err_n1024_m1024.txt
#SBATCH --ntasks=2048
...
echo "Running the ping-pong test with N=1024 and M=1024"
mpirun -np 1024 ../12_ping_pong.exe oce t12e LR : -np 1024 ../12_ping_pong.exe atm icoh U > ../outputs/ocean_times_n1024_m1024.txt


# If interpolation is enabled, extract timing
if [[ "$1" == "true" ]]; then
    # Add the time taken for interpolation to output
    grep "compute" xios_client_*.out | awk -F " " '{print $8}'  > ../outputs/interpolations_times_n1024_m1024.txt
fi