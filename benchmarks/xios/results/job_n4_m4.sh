...
#SBATCH --output=results/out_n4_m4.txt
#SBATCH --error=results/err_n4_m4.txt
#SBATCH --ntasks=8
...
echo "Running the ping-pong test with N=4 and M=4"
mpirun -np 4 ../12_ping_pong.exe oce t12e LR : -np 4 ../12_ping_pong.exe atm icoh U > ../outputs/ocean_times_n4_m4.txt


# If interpolation is enabled, extract timing
if [[ "$1" == "true" ]]; then
    # Add the time taken for interpolation to output
    grep "compute" xios_client_*.out | awk -F " " '{print $8}'  > ../outputs/interpolations_times_n4_m4.txt
fi