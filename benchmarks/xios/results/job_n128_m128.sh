...
mpirun -np 128 ../12_ping_pong.exe oce t12e LR : -np 128 ../12_ping_pong.exe atm icoh U > ../outputs/ocean_times_n128_m128.txt


# If interpolation is enabled, extract timing
if [[ "$1" == "true" ]]; then
    # Add the time taken for interpolation to output
    grep "compute" xios_client_*.out | awk -F " " '{print $8}'  > ../outputs/interpolations_times_n128_m128.txt
fi