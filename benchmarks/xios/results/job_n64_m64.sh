...
#SBATCH --output=results/out_n64_m64.txt
#SBATCH --error=results/err_n64_m64.txt
#SBATCH --ntasks=128
...
echo "Running the ping-pong test with N=64 and M=64"
mpirun -np 64 ../12_ping_pong.exe oce t12e LR : -np 64 ../12_ping_pong.exe atm icoh U > ../outputs/ocean_times_n64_m64.txt


# If interpolation is enabled, extract timing
if [[ "$1" == "true" ]]; then
    # Add the time taken for interpolation to output
    grep "compute" xios_client_*.out | awk -F " " '{print $8}'  > ../outputs/interpolations_times_n64_m64.txt
fi