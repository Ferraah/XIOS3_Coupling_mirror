...
#SBATCH --output=results/out_n2_m2.txt
#SBATCH --error=results/err_n2_m2.txt
#SBATCH --ntasks=4
...
mpirun -np 2 ../12_ping_pong.exe oce t12e LR : -np 2 ../12_ping_pong.exe atm icoh U > ../outputs/ocean_times_n2_m2.txt


# If interpolation is enabled, extract timing
if [[ "$1" == "true" ]]; then
    # Add the time taken for interpolation to output
    grep "compute" xios_client_*.out | awk -F " " '{print $8}'  > ../outputs/interpolations_times_n2_m2.txt
fi