...
def parse_interpolation(n, m, OUTPUT_DIR="outputs"):
    output_file = f"{OUTPUT_DIR}/interpolations_times_n{n}_m{m}.txt"
    yac_times = []
    with open(output_file) as f:
        for line in f:
...
def make_plots(csv_path = "results/scaling_results.csv"):
    ...
    plt.title('XIOS Ping Pong Benchmark Results')
    plt.legend()
    plt.grid()
    ...
    plt.figure(figsize=(10, 6))
    plt.errorbar(df['n'], df['avg_interp'], label='XIOS interpolation', fmt='o')
    plt.xlabel('N (Number of Processes)')
    plt.ylabel('Time (seconds)')
    plt.title('XIOS interpolation Benchmark Results')
    plt.legend()
    plt.grid()
