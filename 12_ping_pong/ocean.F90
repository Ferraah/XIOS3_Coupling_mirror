...
contains
    ...
    subroutine load_coupling_conf(config)
        ...
        CALL xios_get_field_attr("oce_to_atm", freq_op=tmp2)
    ...
    implicit none 
        ...
        call init_field2d_gulfstream(field_desc%ni_glo, field_desc%nj_glo, field_desc%lon, field_desc%lat, field_desc%mask, field_send_original)
        ...
        do while (config%curr_date < config%end_date)
            ...
            call xios_send_field("field2D_send_oce", field_send)
            call xios_recv_field("field2D_recv_oce", field_recv)
